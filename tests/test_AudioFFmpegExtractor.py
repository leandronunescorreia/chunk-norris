...
class TestAudioFFmpegExtractor(unittest.TestCase):
    ...
    def test_get_command_with_filter_complex_mixed(self):
        ...
        expected_command = [
            ...
            file_name,
            "-filter_complex",
            "[0:a:0]pan=mono|c0=0.5*c8+0.5*c9[aout]",
            "-map",
            "[aout]",
            "-f",
            "s16le",
        ...
        command = self.extractor.get_command_with_filter_complex(
            file_name, audio_track, sampling, mixed
        )
    ...
    def test_get_command_with_filter_complex_non_mixed(self):
        ...
        expected_command = [
            ...
            file_name,
            "-filter_complex",
            "[0:a:7][0:a:8]amerge=inputs=2,pan=mono|c0=0.5*c0+0.5*c1[aout]",
            "-map",
            "[aout]",
            "-f",
            "s16le",
    ...
    def test_with_mono_m4a_file(self):
        ...
        audio_buffer = self.extractor.extract(
            file_name=audio_path, audio_track=[0], sampling=16000, mixed=False
        )
        self.assertGreater(len(audio_buffer), 0)
        self.assertIsInstance(audio_buffer, bytes)
        self.assertIsInstance(audio_buffer[0], int)
